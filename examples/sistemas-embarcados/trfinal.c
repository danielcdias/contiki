...
AUTOSTART_PROCESSES(&pwmled, &adc_sensor_test);
...
PROCESS_THREAD(adc_sensor_test, ev, data)
{
    ...
    static int valuePercent, prevValuePercent = 0;
    ...
    while (1) {
        ...
        if (prevValuePercent != valuePercent) {
            process_post(&pwmled, DIMER_CHANGE_EVENT, valuePercent);
        }
        prevValuePercent = valuePercent;
